...
X11R7.6 is an Open Source version of the X Window System that supports many
UNIX® and UNIX-like operating systems (such as Linux, FreeBSD, NetBSD, OpenBSD,
and Solaris) on a variety of platforms. This version is compatible with X11R7.5
...
X11R7.6 is the seventh release in the X11R7 series. Tarballs for packages that
have had updates since the 7.5 release are included in this release. To build
this release, you will first need to get the packages from X11R7.5 and then add
the new packages from X11R7.6 to them.
...
The Release Notes has a table showing the drivers provided with X11R7.6, and
...
invited to complete a membership application. The X11R7.6 release has been
...
xorg@lists.freedesktop.org>. While the current licenses are all open source
licenses, the X.Org Foundation is attempting, with time, to bring as much as
...
Information about X11R7.6 can be found from the X.Org Foundation wiki, and at
mirrors of this server.

X11R7.6

This is the seventh release of the new modular source code tree. The source
...
packages that can be configured, built and installed separately. Please see an
X11R7.6 release site for a complete list of the tarballs.
